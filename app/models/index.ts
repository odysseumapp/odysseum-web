...
export interface DocumentSummary {
  ...
  /** `threads` lists the thread documents (in the Threads folder) this document belongs to. */
  kind: DocumentKind; characters: string[]; locations: string[]; threads: string[];
}
...
export type ThreadAxis = 'rows' | 'columns'
/** `threads` are the thread documents shown in the folder's Threads view; `threadAxis` is where they run (rows by default). */
export interface FolderLayout { pinnedView: FolderView | null; itemOrder: string[]; threads: string[]; threadAxis: ThreadAxis | null }
...
export interface MetadataFields { title: string; synopsis: string; notes: string; status: DocumentStatus; wordGoal: number; characters: string[]; locations: string[]; threads: string[] }
