...
test('every folder supports threads, stacks, independent positions and dragging', async ({ page }) => {
  const info = await createProject(page.request)
  const first = await createDoc(page.request, info.slug, 'Revelation', 'Characters/Race')
  const second = await createDoc(page.request, info.slug, 'Discovery', 'Characters/Class')
  await createDoc(page.request, info.slug, 'Nested point', 'Characters/Race/Nested')
  await page.goto(projectUrl(info.slug))
  ...
  const race = page.getByRole('region', { name: 'Race thread', exact: true })
  const other = page.getByRole('region', { name: 'Class thread', exact: true })
  const point = race.locator(`[data-item-key="${first.document.id}"]`)
  await race.getByRole('button', { name: 'Move Revelation right', exact: true }).click()
  await expect(point).toHaveAttribute('data-position', '2')
  await expect(other.locator(`[data-item-key="${second.document.id}"]`)).toHaveAttribute('data-position', '0')
  ...
  await page.mouse.up()
  await expect(point).toHaveAttribute('data-position', '3')
  await expect.poll(async () => (await project(page.request, info.slug)).folders.find(folder => folder.path === 'Characters/Race')?.positions[first.document.id]).toBe(3)
  await page.screenshot({ path: '.test-data/threads.png', fullPage: true })
  await race.getByRole('button', { name: 'Open folder Nested', exact: true }).dblclick()
  await expect(page.getByRole('tab', { name: 'Threads', exact: true })).toHaveAttribute('aria-selected', 'true')
  await expect(page.getByRole('region', { name: 'Nested thread', exact: true })).toContainText('Nested point')
  await page.getByRole('region', { name: 'Threads', exact: true }).getByRole('button', { name: 'Open Nested point', exact: true }).click()
  await editor(page).fill('A document in a nested thread.')
  await expect.poll(async () => {
    const doc = (await project(page.request, info.slug)).documents.find(doc => doc.title === 'Nested point')!
    return readFile(path.resolve('.test-data/workspace', info.slug, doc.path), 'utf8')
  }).toContain('A document in a nested thread.')
})
...
test('offline reload preserves edits, folders, pins and newly positioned documents', async ({ page, context }) => {
  const info = await createProject(page.request)
  ...
  await newFolder(page, 'Offline thread')
  await page.getByRole('tab', { name: 'Threads', exact: true }).click()
  await page.getByRole('button', { name: 'Pin view for this folder' }).click()
  await page.getByRole('button', { name: 'Add document to Offline thread', exact: true }).click()
  await page.getByRole('dialog').getByRole('textbox', { name: /^Title/ }).fill('Offline point')
  await page.getByRole('button', { name: 'Create document', exact: true }).click()
  await expect(page.getByRole('dialog')).toBeHidden()
  const point = page.getByRole('region', { name: 'Offline thread thread', exact: true }).locator('[data-position]')
  await page.getByRole('button', { name: 'Move Offline point right', exact: true }).click()
  await expect(point).toHaveAttribute('data-position', '1')
  await page.reload()
  await chooseSection(page, 'Threads')
  await expect(point).toHaveAttribute('data-position', '1')
  ...
  await expect.poll(async () => {
    const current = await project(page.request, info.slug)
    const folder = current.folders.find(folder => folder.path === 'Threads/Offline thread')
    const doc = current.documents.find(doc => doc.title === 'Offline point')
    return folder && doc ? { position: folder.positions[doc.id], path: doc.path } : null
  }, { timeout: 20000 }).toEqual({ position: 1, path: 'Threads/Offline thread/Offline point.md' })
