...
test('every folder has a grid of its documents against another folder, with collapsible groups', async ({ page }) => {
  ...
  await page.keyboard.press('Escape')
  await expect(grid.getByRole('button', { name: 'Edit synopsis of Discovery', exact: true })).toHaveText('Add a synopsis')
  await grid.getByRole('button', { name: 'Edit synopsis of Discovery', exact: true }).click()
...
test('the appearance dialog repaints the app, keeps themes on the server and remembers the choice', async ({ page }) => {
  ...
  await dialog.getByRole('button', { name: 'Primary rose', exact: true }).click()
  await expect.poll(primary).not.toBe(before)
  const rose = await primary()
  ...
  await expect.poll(primary).toBe(before)
  await dialog.getByRole('button', { name: 'Use theme Dusk', exact: true }).click()
  await expect.poll(primary).toBe(rose)
  await dialog.getByRole('button', { name: 'Delete Dusk', exact: true }).click()
